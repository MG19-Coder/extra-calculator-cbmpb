import { BarChart3, CalendarDays, ClipboardCheck, FileText, ListChecks, Menu, PlusCircle, Settings, Sparkles } from "lucide-react";
...
type Tab = "dashboard" | "lancamentos" | "novo" | "escala" | "calendario" | "relatorio" | "contracheque" | "config" | "feriados";
...
function App() {
  ...
  const monthItems = state.lancamentos.filter((item) => item.competenciaImplantacao === state.selectedMonth || item.competenciaServico === state.selectedMonth);
  ...
  const content = {
    ...
    calendario: <CalendarMonth competencia={state.selectedMonth} items={state.lancamentos} feriados={state.feriados} valores={state.valores} onUpdate={store.updateLaunch} />,
    relatorio: <Report state={state} totals={totals} />,
    contracheque: <Paycheck state={state} totals={totals} onSave={store.upsertPaycheck} />,
    config: <SettingsPage state={state} onValues={store.updateValues} onMilitar={store.updateMilitar} />,
      ...
      <header className="border-b border-slate-200 bg-ink text-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-300">{getMonthName(state.selectedMonth)}</p>
          </div>
